package com.auroras.module.charge.saver.aurorasprotectservice;
...
import com.auroras.module.charge.saver.AurorasBatteryProtectActivity;
...
import com.auroras.module.charge.saver.aurorasutils.WidgetContainer;
...
public class ServiceBattery extends Service {
    ...
    private void showChargeView() {
        boolean isChargeScreenSaver = (boolean) AurorasUtils.readData(this, BatteryConstants.CHARGE_SAVER_SWITCH, true);
        if (!isChargeScreenSaver) {
            return;
        }
        if ((Boolean) AurorasUtils.readData(this, BatteryConstants.IS_ACTIVITY, true)) {
            if (entry == null || !entry.isCharging()) {
                return;
            }
            Intent intent = new Intent().addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            if (AurorasUtils.readData(this, BatteryConstants.KEY_SAVER_TYPE, BatteryConstants.TYPE_HOR_BAR).equals(BatteryConstants.TYPE_HOR_BAR)) {
                intent.putExtra("type", "bar");
            } else {
                intent.putExtra("type", "duck");
            }
            intent.setClass(this, AurorasBatteryProtectActivity.class);
