...
public class SecuritySurface extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public SecuritySurface(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public SecuritySurface(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public SecuritySurface(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    boolean startCamera() {
        if (!SecurityMyPref.fetchIntruder()) return false;
        try {
                        ...
                        public void onPreviewFrame(final byte[] data, final Camera cam) {
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        if (soundPool != null) {
                                            if (SecurityPreference.isShutterSoundEnabled()) {
                                        ...
                                        if (IntruderApi.addIntruder(bitmap, pkgName)) {
                                            SecurityMyPref.setHasIntruder(true);
                                        }
