...
public class Pref {
    ...
    public Pref useNormalPasswd(boolean normal) {
...
//        return App.getSharedPreferences().getBoolean("nor", false);
    ...
    public Pref setPasswd(String pass, boolean normal) {
...
//        String string = App.getSharedPreferences().getString(normal ? "pp" : "pg", null);
    ...
    public Pref migComplete(String tag) {
        editor.putBoolean(tag, true);
        return this;
    }

    public Pref putBoolean(String tag, boolean value) {
        editor.putBoolean(tag, value);
        return this;
    }

    public Pref remove(String tag) {
        editor.remove(tag);
        return this;
    }

    public Pref putString(String tag, String value) {
        editor.putString(tag, value);
        return this;
    }

    public Pref putInt(String tag, int value) {
        editor.putInt(tag, value);
        return this;
    }

    public Pref putLong(String tag, long value) {
        editor.putLong(tag, value);
        return this;
    }

    public Pref putFloat(String tag, float value) {
    ...
    public static void pressMenu(int menuId) {
        ...
        int redDotCount = 0;
        for (int i = 1; i < MyMenu.newidkeys.length; ++i) {
            if (i != menuId && !sp.getBoolean(MyMenu.newidkeys[i], false)) {
                ++redDotCount;
            }
        }
        if (redDotCount == 0) {
        ...
        e.putBoolean(MyMenu.newidkeys[menuId], true).apply();
    }

    public static boolean isMenuPressed(int menuId) {
        return App.getSharedPreferences().getBoolean(MyMenu.newidkeys[menuId], false);
    }

    public static void clearMenuPressedState(int menuId) {
        App.getSharedPreferences().edit().putBoolean(MyMenu.newidkeys[menuId], false).putBoolean("reddot", true).apply();
    }

    public static void clearMenuPressedStateWithOptions(int menuId, int optId) {
        App.getSharedPreferences().edit().putBoolean(MyMenu.newidkeys[menuId], false).putBoolean(setting_reddot_key[optId], false).putBoolean("reddot", true).apply();
    }
    ...
    public static void launchNow() {
        ...
        if (count < 23) {
            if (count == 12 && !hasOption(OPT_RATE_REDDOT)) {
                clearMenuPressedStateWithOptions(MyMenu.MENU_SETTING, OPT_RATE_REDDOT);
            } else if (count == 15 && !hasOption(OPT_ADVANCE_REDDOT)) {
                clearMenuPressedStateWithOptions(MyMenu.MENU_SETTING, OPT_ADVANCE_REDDOT);
            } else if (count == 22 && !hasOption(OPT_NOTIFICATION_REDDOT)) {
                clearMenuPressedStateWithOptions(MyMenu.MENU_SETTING, OPT_NOTIFICATION_REDDOT);
            }
