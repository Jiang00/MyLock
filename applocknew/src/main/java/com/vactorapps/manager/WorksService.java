 ...
 */
public class WorksService extends Service {
    boolean home = false;
    ...
    public static void startWorking(Context context) {
        context.startService(new Intent(context, WorksService.class).putExtra(EXTRA_WORKING, WORKING_TOGGLE_ON));
    }

    public static void stopWorking(Context context) {
        context.startService(new Intent(context, WorksService.class).putExtra(EXTRA_WORKING, WORKING_TOGGLE_OFF));
    }
...
//    }
    ...
    class MyRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (home) {
                if (SavBridgeImpl.ins.context == null) {
                    SavBridgeImpl.ins.context = WorksService.this;
                }
                SavBridgeImpl.ins.detachFromWindow(SavBridgeImpl.MENU_IDX_ALL);
            }
        }
    }
    ...
    class LockTask extends Thread {
        ...
        public void run() {
            ...
            while (running) {
                ...
                final String packageName = getLauncherTopApp(WorksService.this, mActivityManager);
...
//                        }
                        ...
                        switch (slot) {
                            case VacPref.PREF_BRIEF_EVERY_TIME:
                                tmpUnlockedApps.clear();
                                getSharedPreferences("tmp", MODE_PRIVATE).edit().remove(VacPref.PREF_TMP_UNLOCK).commit();
                                break;
                            default:
                ...
                if (!alert) {
                    if (!lockApps.containsKey(packageName)) {
                        if (asks.containsKey(packageName) && VacPref.requireAsk()) {
                        ...
                        if (briefTimes.containsKey(packageName)) {
                            int slot = MyApp.getSharedPreferences().getInt(VacPref.PREF_BRIEF_SLOT, VacPref.PREF_DEFAULT);
                            if (slot != VacPref.PREF_BRIEF_EVERY_TIME) {
                                int time = 300;
                                if (slot != VacPref.PREF_BRIEF_5_MIN) {
...
////                v.setPadding(0, Utils.getDimens(WorksService.this, 16), 0, 0);
////            }
//            wm.addView(v, lp);
...
//        };
        private void alertForUnlock(final String packageName) {
            lastApp = packageName;
            alterShow = true;
            if (Utils.hasSystemAlertPermission(MyApp.getContext())) {
                alert = true;
                SavBridgeImpl.reset(WorksService.this, false, true, packageName);
    ...
    IWorker.Stub binder = new IWorker.Stub() {
        ...
        public void updateProtectStatus() throws RemoteException {
            sleep = VacPref.isProtectStopped();
            if (sleep) {
                onStopProtect();
            }
        }
        ...
        public void showNotification(boolean yes) throws RemoteException {
            MyApp.getSharedPreferences().edit().putBoolean("sn", yes).apply();
            WorksService.this.showNotification(yes);
        }
    ...
    public void showWidgetIfNecessary(boolean show) {
        if (MyApp.getSharedPreferences().getBoolean(VacPref.PREF_SHOW_WIDGET, false)) {
    ...
    WidgetVac securityWidget;

    public void notifyLockedAppsUpdate() {
        try {
            long id = SafeDB.defaultDB().getLong(VacPref.PREF_ACTIVE_PROFILE_ID, 0L);
            SQLiteDatabase db = ProfileHelperVac.singleton(MyApp.getContext()).getReadableDatabase();
            Map<String, Boolean> apps = ProfileHelperVac.ProfileEntry.getLockedApps(db, id);
            apps.remove("com.setting.SettingActivity");
            apps.remove("");
            lockApps.clear();
    ...
    public void unlockSuccess(boolean unlockMe) {
        ...
        if (unlockMe) {
            try {
                ...
                ProfileHelperVac.ProfileEntry.deleteLockedApp(SacProfiles.getDB(), profileId, lastApp);
    ...
    public void unlockLastApplication(String app, boolean remove) {
        ...
        getSharedPreferences("tmp", MODE_PRIVATE).edit().putString(VacPref.PREF_TMP_UNLOCK, sb.toString()).commit();
    }
        ...
        if (yes) {
            ...
            if (sleep) {
                ...
                pi = PendingIntent.getService(getApplicationContext(), 0,
                        new Intent(getApplicationContext(), WorksService.class).putExtra(WORK_EXTRA_KEY, WORK_TURN_ON_PROTECT),
                ...
                pi = PendingIntent.getActivity(getApplicationContext(), 0,
                        new Intent(getApplicationContext(), TogglePatternActivity.class)
                                .putExtra("action", TogglePatternActivity.ACTION_TOGGLE_PROTECT)
...
//    };
    ...
    public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            hasAccessUsagePermission = Utils.checkPermissionIsGrant(MyApp.getContext(), Utils.OP_GET_USAGE_STATS) == AppOpsManager.MODE_ALLOWED;
        }
...
//            PendingIntent pi = PendingIntent.getService(this, 0, new Intent(this, WorksService.class).putExtra("alarm", true), PendingIntent.FLAG_UPDATE_CURRENT);
...
//        }
        ...
        String[] unlocked = getSharedPreferences("tmp", MODE_PRIVATE).getString(VacPref.PREF_TMP_UNLOCK, "").split(";");
        ...
        BroadcastReceiver sOnBroadcastReciver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (!MyApp.getSharedPreferences().getBoolean("stop_service", false)) {
        ...
        excludesClasses.put(FristActivity.class.getName(), true);
        excludesClasses.put(TogglePatternActivity.class.getName(), true);
...
//        String pkgName = getTopPackageName();
        ...
        if (getPackageName().equals(pkgName)) {
            ...
            if (!excludesClasses.containsKey(className)) {
                Intent i = new Intent(getApplicationContext(), TogglePatternActivity.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                startActivity(i);
            }
        }
        int slot = MyApp.getSharedPreferences().getInt(VacPref.PREF_BRIEF_SLOT, VacPref.PREF_DEFAULT);
    ...
    public void onDestroy() {
        Utils.LOGE("WorksService is destroying...");
        handler = null;
        try {
            hideAlertImmediate();
        } catch (Exception e) {
