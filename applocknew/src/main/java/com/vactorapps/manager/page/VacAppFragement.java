...
import static com.vactorapps.manager.page.VacThemeFragment.TAG_TOP_AD;

/**
 * Created by SongHualin on 6/24/2015.
 */
public class VacAppFragement extends BaseFragment implements RefreshList, SearchView.OnQueryTextListener {
    ...
    ProfileHelperVac.ProfileEntry profileEntry;
    SQLiteDatabase db;

    static View headerView;

    SharPFiveVac shareFive;
    ...
    protected void onArguments(Bundle arguments) {
        profileEntry = new ProfileHelperVac.ProfileEntry();
    ...
    private void updateLocks() {
        db = ProfileHelperVac.singleton(getActivity()).getWritableDatabase();
        if (profileEntry.name != null) {
            try {
                locks = ProfileHelperVac.ProfileEntry.getLockedApps(db, profileEntry.id);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        shareFive = new SharPFiveVac(getActivity());
    ...
    public void saveOrCreateProfile(String profileName, IWorker server) {
        ...
        try {
            if (profileEntry.name == null) {
                profileEntry.id = ProfileHelperVac.ProfileEntry.createProfile(db, profileName, list);
                profileEntry.name = profileName;
                SacProfiles.addProfile(profileEntry);
            } else {
                ProfileHelperVac.ProfileEntry.updateProfile(db, profileEntry.id, list);
                if (server != null) {
